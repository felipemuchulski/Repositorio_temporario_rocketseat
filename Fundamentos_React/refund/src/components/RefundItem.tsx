import { Link } from "react-router";

export type RefundItemProps = {
  id: string;
  name: string;
  category: string;
  amount: string;
  categoryImg: string;
};

type Props = React.ComponentProps<typeof Link> & RefundItemProps;

export function RefundItem({
  id,
  name,
  category,
  amount,
  categoryImg,
  ...rest
}: Props) {
  
  
    return (
    <Link
      className="flex items-center gap-3 hover:bg-green-100 cursor-pointer rounded-md p-2"
      {...rest}
    >
      <img src={categoryImg} alt="Ícone da categoria" className="w-8 h-8" />

      <div className="flex flex-col flex-1">
        <strong className="text-sm text-gray-100">{name}</strong>
        <span className="text-xs text-gray-200">{category}</span>
      </div>

      <span className="text-sm text-gray-100 font-semibold">
        <small className="font-normal text-gray-200">R$ </small>
        {amount}
      </span>
    </Link>
  );
}
